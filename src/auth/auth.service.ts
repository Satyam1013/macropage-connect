...
export class AuthService {
  ...
  async oauthLogin(dto: OAuthDto): Promise<AuthResponse> {
  ...
  private buildAuthResponse(user: any, message: string): AuthResponse {
    const accessToken = this.signAccessToken(user.id, user.email);
  ...
  private signAccessToken(sub: string, email: string): string {
    const expiresIn = (this.config.get<string>('JWT_EXPIRES_IN', '15m')) as `${number}${'s'|'m'|'h'|'d'}`;
